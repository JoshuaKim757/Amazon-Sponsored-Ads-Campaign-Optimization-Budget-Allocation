...
KPI를 사전에 설정해 캠페인 종료 후 결과에 맞춰 성공 기준을 변경하지 않도록 했습니다.
...
초기에는 총 15개 경쟁사 ASIN을 대상으로 설정했습니다.
...
캠페인이 진행되면서 단순 주문 여부만 보는 방식에서 벗어나 ROAS를 추가 판단 기준으로 사용했습니다.
...
이 프로젝트에서 가장 중요했던 것은 처음 세운 전략을 끝까지 유지하는 것이 아니었습니다.
...
광고를 클릭하게 만드는 것과 실제 구매를 만드는 것은 다른 문제이며, 캠페인 운영에서는 처음 세운 전략보다 실제 성과에 따라 예산과 입찰 기준을 계속 수정하는 것이 중요하다는 점입니다.
...
과정 수료와 프로젝트 경험은 현재 유효한 자격증과 구분해 포트폴리오에 기록합니다.
...
### README에 넣을 시각화도 이렇게 추천해

이 프로젝트는 **4개면 충분해.**

1. `01_final_kpi.png`
   - CTR / CVR / Sales의 **Target vs Actual**
   - 가장 중요함.

2. `02_budget_reallocation.png`
   - `50:50 → 75:25 → 85:15`
   - 동시에 Keyword vs PDP Orders를 보여주면 제일 좋음.
   - 사실상 이 프로젝트의 **대표 차트**로 추천.

3. `03_keyword_performance.png`
   - Top Keywords와 Orders.

4. `04_pdp_performance.png`
   - PDP별 Orders.
   - 가능하면 성공/저성과 Target 비교.

그리고 기존 PPT 페이지를 캡처해서 그대로 넣기보다는 **차트를 새로 만들어서 넣는 것**을 추천해. PPT 디자인 자체는 나쁘지 않은데, 그대로 쓰면 대학 수업 발표 프로젝트 느낌이 강해져. 반대로 데이터만 꺼내서 깔끔한 차트 4개로 다시 만들면 다른 GitHub 프로젝트들과 훨씬 자연스럽게 통일돼.

특히 이 프로젝트의 대표 이미지는 **“Keyword vs PDP 주문 + Budget Allocation 변화” 한 장**으로 만드는 게 제일 좋아. 그 한 장이 이 프로젝트에서 네가 실제로 한 일을 거의 전부 설명해준다.
